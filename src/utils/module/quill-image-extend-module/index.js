...
export const QuillWatch = {
  ...
  on(imageExtendId, ImageExtend) {
    // 登记注册使用了 ImageEXtend 的编辑器
    if (!this.watcher[imageExtendId]) {
      this.watcher[imageExtendId] = ImageExtend
    }
  },
  emit(activeId, type = 1) {
    // 事件发射触发
...
export class ImageExtend {
  ...
  uploadImg() {
    const self = this
    // eslint-disable-next-line no-unused-vars
    const quillLoading = self.quillLoading
  ...
  insertImg() {
    ...
    const cursorIndex = self.quill.selection.lastRange.index // 插入图片
   ...
   */
  progress(pro) {
    const eleUploadBox = document.querySelector('.quil-editor-upload')
    if (eleUploadBox) {
      eleUploadBox.innerHTML = `上传中 ${pro}`
    }
  }
